...
class Minesweeper:
        ...
        for r,c in self.mines:
            for i in range(-1, 2):
                for j in range(-1, 2):
                    cell_i=r+i
                    cell_j=c+j
                    if (cell_i<0 or cell_j<0 or cell_i>=self.rows or cell_j>=self.cols or self.board[cell_i][cell_j]=='💣'):
                        continue
    ...
    def get_board(self) -> list:
        ...
        board = [[self.board[i][j] if (i,j) in self.revealed  else ""  for j in range(self.cols)] for i in range(self.rows)]
        return board
